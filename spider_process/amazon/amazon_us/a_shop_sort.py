...
write_file_all = open("X:\数据库\美国亚马逊\{3_3店铺分类}[id,main_sales,sort].txt", "w", encoding="utf-8")
shop_set = set()
if os.path.exists("X:\数据库\美国亚马逊\{{amazon_sortshop-data_{}}}[url,id,name,num,sort].txt".format(month)):
    write_file = open("X:\数据库\美国亚马逊\{{3_3店铺分类_{}}}[id,main_sales,sort].txt".format(month), "w", encoding="utf-8")
    with open("X:\数据库\美国亚马逊\{{amazon_sortshop-data_{}}}[url,id,name,num,sort].txt".format(month), "r",encoding="utf-8",errors="ignore") as f:
        for i in f:
            try:
                data = i.strip().split(",")
                shop_id = data[1]
...
with open("X:\数据库\美国亚马逊\{}\{{3_3店铺分类}}[id,main_sales,sort].txt".format(last_month), "r", encoding="utf-8") as f:
    for i in f:
        shop_id = i.split(",")[0]
        if shop_id not in shop_set:
            shop_set.add(shop_id)
