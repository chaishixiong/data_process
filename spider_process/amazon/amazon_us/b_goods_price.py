...
last_month = 202212
write_file=open("X:\数据库\美国亚马逊\{3_2店铺单价}[id,price_avg].txt","w",encoding="utf-8")
with open("X:\数据库\美国亚马逊\{{amazonus_goodsinfo_{}}}[shop_id,goods_name,comment_score,comment_num,goods_url,price,goods_id,goods_num,brand].txt".format(month),"r",encoding="utf-8") as f:
    for i in f:
        data = i.strip().split(",")
        shop_id = data[0]
        try:
...
for i,y in shop_dict.items():
    ...
    if totlexomment:
        ...
        amazon_id.add(i)
        write_file.write("{},{}\n".format(i,price_avg))
with open(r"X:\数据库\美国亚马逊\{}\{{3_2店铺单价}}[id,price_avg].txt".format(last_month),"r",encoding="utf-8") as f:
    for i in f:
        data = i.strip().split(",")
