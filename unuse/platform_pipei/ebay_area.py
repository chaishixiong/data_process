...
if __name__ == "__main__":
    ...
    sql_get = "SELECT seller_name,seller_text,index_id from ebay_usrinfo_201910 where country like '中国'"
    sql_update = '''update ebay_usrinfo_201910 set province_match = "{}",city_match = "{}",area_match = "{}",match_confidence="{}" where index_id = "{}";'''
    ...
    for data in all_data:
        if data:
            if print_num % 1000 == 0:
                print(print_num)
            seller_name = data.get("seller_name") if data.get("seller_name") else""
            seller_text = data.get("seller_text") if data.get("seller_text") else""
            shop_key = data.get("index_id") if data.get("index_id") else""
            match_text = seller_name+"|"+seller_text

            try:
                match_task.address_match(match_text,shop_key,sql_update,cursor_update)
