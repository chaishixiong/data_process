        ...
        if state < 1:
            for province in self.province_list:
                if match_type:
                    ...
                    if " " + province[0].lower() + " " in match_text.lower():
                        judge = 1
            ...
            if not province_last:
                for city in self.city_list:
                    if match_type:
                        ...
                        if " " + city[1][0].lower() + " " in match_text.lower():
                            judge = 1
                ...
                for city in self.city_list:
                    if match_type:
                        ...
                        if " " + city[1][0].lower()+" " in match_text.lower() and city[0][1] == province_last:
                            judge = 1
                            province_match = city[0][1]
                            city_match = city[1][1]
                            state = 2
            ...
            for county in self.county_list:
                if state == 1:
                    if match_type:
                        ...
                        if " " + county[2][0].lower() in match_text.lower() and county[0][1] == province_m:
                    ...
                    if match_type:
                        ...
                        if " " + county[2][0].lower() + " " in match_text.lower() and county[1][1] == city_m:
                            area_match = county[2][1]
                            judge = 1
                            state = 3
                            break
                    ...
                    if match_type:
                        ...
                        if " " + county[2][0].lower() + " " in match_text.lower():
...
if __name__ == "__main__":
    match_task = p_pipei()
    match_task.get_address("浙江")#浙江中文

    connect_get = match_task.creat_connect()
    cursor_get = connect_get.cursor()
    mouth = input("请输入月份：")
    sql_get = "select index_id,address_detail,address,company_name,province,city from alibabagj_shopinfo_202001 where province like 'zhejiang' or province = ''"
    sql_get = sql_get.replace("202001",str(mouth))
    sql_update = '''update alibabagj_shopinfo_202001 set province_match = "{}",city_match = "{}",area_match = "{}",match_confidence="{}" where index_id = "{}";'''
    sql_update = sql_update.replace("202001",str(mouth))
    ...
    for data in all_data:
        if data:
            if print_num % 1000 == 0:
                print(print_num)
            address_detail = data.get("address_detail") if data.get("address_detail") else""
            address_detail = address_detail.replace(","," ")
            address_detail = address_detail.replace("，"," ")
            address_detail = address_detail.replace("."," ")

            address = data.get("address") if data.get("address") else""
            shop_name = data.get("company_name") if data.get("company_name") else""
            ...
            match_text = " {} {} ".format(province,city)
            match_text2 = " {} {} {} ".format(address_detail,address,shop_name)
            try:
                match_task.address_match(match_text,shop_key,sql_update,cursor_update,match_text2)
