...
if __name__ == "__main__":
    ...
    sql_get = "select dizhi,dizhi2,index_id from smt_zhejiang"
    sql_update = '''update smt_zhejiang set province_match = "{}",city_match = "{}",area_match = "{}",match_confidence="{}" where index_id = "{}";'''
    ...
    for data in all_data:
        if data:
            if print_num % 1000 == 0:
                print(print_num)
            address = data.get("dizhi") if data.get("dizhi") else ""
            # shop_name = data.get("company") if data.get("company") else""

            shop_key = data.get("index_id") if data.get("index_id") else ""
            match_text = " {} ".format(address)
            # match_text2 = shop_name
            # try:
