...
if __name__ == "__main__":
    ...
    sql_get = "SELECT seller_name,company_name,Location,index_id from dhgate_shopinfo_201911"
    sql_update = '''update dhgate_shopinfo_201911 set province_match = "{}",city_match = "{}",area_match = "{}",match_confidence="{}" where index_id = "{}";'''
    ...
    for data in all_data:
        if data:
            if print_num % 1000 == 0:
                print(print_num)
            shop_name = data.get("seller_name") if data.get("seller_name") else""
            company_name = data.get("company_name") if data.get("company_name") else""
            location = data.get("Location") if data.get("Location") else""

            shop_key = data.get("index_id") if data.get("index_id") else""
            match_text = location
            match_text2 = shop_name+"|"+company_name
            try:
