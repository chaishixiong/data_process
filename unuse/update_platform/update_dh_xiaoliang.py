...
sql_select = '''SELECT minPrice,maxPrice,item_sold,shop_id from dhgate_goodsinfo_201907
where orders != "" and orders is not null and item_sold !="0"
'''
...
sql_update = '''UPDATE dhgate_shopinfo_201907 set transactions_j = "{}",sales_volume_j = "{}" where shop_id = "{}"'''
    ...
    try:
        ...
        if minPrice:
            if maxPrice and maxPrice != "0":
                try:
                    a = int(sales_num)*(float(minPrice)+float(maxPrice))/2
                    data_dict[shop_id].append((int(sales_num), a))
                except:
