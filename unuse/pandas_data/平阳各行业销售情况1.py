...
def get_goods():
    ...
    for i in range(10,8,-1):
        print(i)
        term = pd.read_sql("select goods_id,goods_name,shop_id,cid,sales_month,discount_price from tmall_goodsmobile_2019{0} limit 10000".format("%02d" % i),con=engine_tmall)
        print(len(term))
