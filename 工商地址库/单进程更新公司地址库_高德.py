#!/usr/bin/env Python
# coding=utf-8
import time, asyncio, aiohttp, requests, json, jsonpath, re, socket, datetime, redis, pymysql
from tools.tools_d.Bloomfilter import BloomFilter
...
class GetTask(object):
    def __init__(self, table):
        self.connect = pymysql.connect(host='192.168.1.4', port=3306, database='oridata', user='update_all',
                                       password='123456',
        ...
        self.r = redis.Redis(host='127.0.0.1', port=6379, db=4, decode_responses=True,password="nriat.123456")
        ...
        self.ak = '4217249ade242a808a9cae42a38bea24'
        self.table = table
        self.gevent_num = 60
        # 信号量，控制协程数，防止爬的过快
    ...
    def main_get_html_one(self, get_CompanyAndCompanyAddr_list):
        ...
        for CompanyAdd in get_CompanyAndCompanyAddr_list:
            self.get_html_one(CompanyAdd)
            # 解析
    ...
    def main_parse_html_1(self):
        ...
        for html in self.JX_1_htmls:
            self.multi_parse_html_one(html)
        # 完成解析清除当前已解析过的数据
    ...
    # 提交请求获取网页html
    def get_html_one(self, CompanyAndCompanyAddr):
        try:
            ...
            if self.bf2.isContains(CompanyAndCompanyAddr):
                ...
                if company_addr != '':
                    address1 = parse.quote(company_addr, safe='/')
                    jwd_url = 'https://restapi.amap.com/v3/geocode/geo?address={}&output=json&key={}&output=json'.format(
                        address1, self.ak)
                    # async with是异步上下文管理器
                    r = requests.get(jwd_url, headers=self.header)
                    # r = request.get('GET', jwd_url, headers=self.header) as resp:  # 提出请求
                    html = r.text  # 直接获取到bytes
                            # 将未解析的数据添加进列表
    ...
    def main_get_html_two(self):
        try:
            # loop = asyncio.get_event_loop()  # 获取事件循环
            for CompanyAndCompanyAddr_html in self.tasks:  # 把所有任务放到一个列表中
                self.get_html_two(CompanyAndCompanyAddr_html)
            # loop.run_until_complete(asyncio.wait(tasks))  # 激活协程
            # 解析
    ...
    def get_html_two(self, CompanyAndCompanyAddr_location):
        ...
        try:
            ...
            r_2 = requests.get(address_url, headers=self.header)
            html = r_2.text  # 直接获取到bytes
            # 将未解析的数据添加进列表
    ...
    def main_parse_html_2(self):
        ...
        for html in self.JX_2_htmls:
            self.multi_parse_html_two(html)
        # 完成解析清除当前已解析过的数据
    ...
    def BuChong(self, address):
        address1 = parse.quote(address, safe='/')
